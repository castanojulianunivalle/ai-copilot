/**
 * AISCOP-9: Pruebas E2E flujo registro
 * Cubre: validación de campos, fortaleza de contraseña, flujo completo de registro
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';

// --- Helpers de validación (extraídos del componente LoginRegister) ---
...
describe('Validación de email', () => {
  ...
  it('falla si el email no tiene dominio', () => {
...
describe('Fortaleza de contraseña', () => {
  ...
  it('es Débil con solo letras minúsculas cortas', () => {
    expect(getPasswordStrength('abc')).toBe('Débil');
  });

  it('es Regular con longitud y mayúscula pero sin número ni especial', () => {
    expect(getPasswordStrength('Abcdefgh')).toBe('Regular');
  });
  ...
  it('es Fuerte con longitud, mayúscula, número y carácter especial', () => {
    expect(getPasswordStrength('Abcdef1!')).toBe('Fuerte');
  });
});

// --- Pruebas del flujo de registro (mock de onSignUp) ---
...
describe('Flujo de registro', () => {
  ...
  it('no llama onSignUp si la contraseña es débil', async () => {
    const email = 'test@test.com';
    const password = 'debil';
    if (validateEmail(email) || validatePassword(password)) return;
    await mockSignUp(email, password);
    expect(mockSignUp).not.toHaveBeenCalled();
  });

  it('no llama onSignUp si las contraseñas no coinciden', async () => {
    const email = 'test@test.com';
    const password = 'Valida123';
    const confirm = 'Diferente123';
  ...
  it('llama onSignUp con credenciales válidas', async () => {
    const email = 'test@test.com';
    const password = 'Valida123';
    const confirm = 'Valida123';
    mockSignUp.mockResolvedValue({});
    if (!validateEmail(email) && !validatePassword(password) && password === confirm) {
      await mockSignUp(email, password);
    }
