              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full rounded-lg border px-4 py-2 pr-10 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${
                    emailError ? 'border-red-500' : email && !emailError ? 'border-green-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
                  autoComplete="email"
                  disabled={loading}
                />
                {touched.email && email && (
                  <span className="absolute right-3 top-2.5">
                    {emailError ? <XCircle className="w-4 h-4 text-red-500" /> : <CheckCircle className="w-4 h-4 text-green-500" />}
                  </span>
                )}
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full rounded-lg border px-4 py-2 pr-10 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${
                    passwordError ? 'border-red-500' : password && !passwordError ? 'border-green-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
                  autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
                  disabled={loading}
                />
                <button type="button" onClick={() => setShowPassword((s) => !s)}
                  className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" tabIndex={-1}>
              ...
              {mode === 'register' && password && (
                <div className="mt-2">
                  ...
                  {strength.label && (
                    <p className="text-xs mt-1 text-gray-500 dark:text-gray-400">Fortaleza: <span className="font-medium">{strength.label}</span></p>
                  )}
                </div>
              )}
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className={`w-full rounded-lg border px-4 py-2 pr-10 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${
                      confirmError ? 'border-red-500' : confirmPassword && confirmPassword === password ? 'border-green-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
